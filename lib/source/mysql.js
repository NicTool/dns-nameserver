 ...
 *
 * Does not self-detect changes: the 2.x schema carries no maintained change
 * feed. nt_zone_record.timestamp has no ON UPDATE clause and the API does not
 * set it, and the *_log tables are never written, so the only way to notice an
 * in-place rdata edit is to re-read everything — which is the publish itself.
 *
 * So a publishing nameserver on this source wants interval > 0. For event mode,
 * whoever writes the record calls notifyZoneChanged() (see
 * NameserverSupervisor.notifyZoneChanged).
 */
