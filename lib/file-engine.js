...
export class FileEngine extends Nameserver {
  ...
  async start() {
    if (!this.publisher) {
      throw new Error(
        `${this.name ?? this.engine}: publisher is required for file engines`,
      )
    }
    await super.start()
  }
