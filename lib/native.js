...
function buildRR(name, typeId, ttl, rdata) {
  ...
  out.writeUInt16BE(typeId, o); o += 2
  out.writeUInt16BE(CLASS_IN, o); o += 2
  out.writeUInt32BE(ttl >>> 0, o); o += 4
  out.writeUInt16BE(rdata.length, o); o += 2
  rdata.copy(out, o)
  return out
}
...
function encodeRecord(rec, owner, apex, zone) {
  ...
  switch (type) {
    ...
    case 'SRV':
      return buildRR(owner, TYPE.SRV, ttl, rdataSrv(rec.priority ?? 0, rec.weight ?? 0, rec.port ?? 0, addr))
    case 'CAA':
      return buildRR(owner, TYPE.CAA, ttl, rdataCaa(rec.weight ?? 0, rec.tag ?? 'issue', addr))
    default:
      return null
  }
}
...
function buildError(id, flags, questionRaw, rcode) {
  return buildResponse({ id, flags, questionRaw: questionRaw ?? Buffer.alloc(0), answers: [], rcode })
}
...
export class NativeNS extends Nameserver {
  ...
  boundAddresses() {
    return this._servers.map((s) => (typeof s.address === 'function' ? s.address() : null))
  }
  ...
  handleQuery(buf) {
    ...
    for (const rec of records) {
      ...
      const typeMatch =
        qtype === TYPE.ANY || recType === typeIdToName(qtype)
    ...
    if (qname === apex && (qtype === TYPE.SOA || (qtype === TYPE.ANY && answers.length === 0))) {
      const soaRec = records.find((r) => String(r.type ?? '').toUpperCase() === 'SOA')
      if (soaRec) {
        answers.push(encodeRecord(soaRec, apex, apex, zone))
      } else if (zone) {
        answers.push(buildRR(apex, TYPE.SOA, Number(zone.ttl ?? 3600), rdataSoa(zone, apex)))
      }
    }
