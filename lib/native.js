...
function readName(buf, offset) {
  ...
  while (pos >= 0 && pos < buf.length) {
    ...
    if ((len & 0xc0) === 0xc0) {
      ...
      if (++jumps > MAX_NAME_JUMPS) break
      if (end === -1) end = pos + 2
      pos = ((len & 0x3f) << 8) | buf[pos + 1]
      continue
    }
    // 0b01/0b10 length prefixes are reserved; only 1-63 is a literal label
    if (len > MAX_LABEL_BYTES) break
  ...
  return { name: labels.join('.'), end: end === -1 ? pos + 1 : end, ok }
}
...
function writeName(name) {
  ...
  const bufs = labels.map((l) => {
    ...
    if (b.length === 0 || b.length > MAX_LABEL_BYTES) {
      throw new Error(`writeName: invalid label length ${b.length} in "${clean}"`)
    }
    return Buffer.concat([Buffer.from([b.length]), b])
  })
  const out = Buffer.concat([...bufs, Buffer.from([0])])
  if (out.length > MAX_NAME_BYTES) {
    throw new Error(`writeName: name exceeds ${MAX_NAME_BYTES} bytes: "${clean}"`)
  }
  return out
}
...
function writeIPv4(addr) {
  ...
  if (parts.length !== 4) throw new Error(`writeIPv4: invalid address "${addr}"`)
  return Buffer.from(
    parts.map((p) => {
      if (!/^\d{1,3}$/.test(p)) throw new Error(`writeIPv4: invalid address "${addr}"`)
      const n = Number(p)
      if (n > 255) throw new Error(`writeIPv4: invalid address "${addr}"`)
      return n
    }),
  )
}
...
function writeIPv6(addr) {
  ...
  if (groups.length !== 8) throw new Error(`writeIPv6: invalid address "${addr}"`)
  const out = Buffer.allocUnsafe(16)
  for (let i = 0; i < 8; i++) {
    if (!/^[0-9a-f]{1,4}$/i.test(groups[i])) {
      throw new Error(`writeIPv6: invalid address "${addr}"`)
    }
...
function rdataTxt(text) {
  ...
  for (let i = 0; i < buf.length; i += 255) {
    const c = buf.subarray(i, i + 255)
    chunks.push(Buffer.concat([Buffer.from([c.length]), c]))
  }
  if (!chunks.length) chunks.push(Buffer.from([0]))
...
function encodeRecord(rec, owner, apex, zone) {
  ...
  switch (type) {
    ...
    case 'SOA':
      return buildRR(owner, TYPE.SOA, ttl, rdataSoa(zone ?? rec, apex))
    case 'SRV':
      return buildRR(
        owner,
        TYPE.SRV,
...
function parsePacket(buf) {
  ...
  const { name, end, ok } = readName(buf, 12)
  if (!ok) return null
...
function buildResponse({ id, flags, questionRaw, answers, rcode = RCODE_NOERROR }) {
  const respFlags =
    ...
    (rcode & 0xf)
  const question = questionRaw ?? Buffer.alloc(0)
  const header = Buffer.allocUnsafe(12)
  header.writeUInt16BE(id, 0)
  header.writeUInt16BE(respFlags, 2)
  header.writeUInt16BE(question.length ? 1 : 0, 4) // QDCOUNT
...
export class NativeNS extends Nameserver {
  ...
  handleQuery(buf) {
    ...
    if (!parsed) {
      const hasHeader = buf.length >= 12
      return buildError(
        hasHeader ? buf.readUInt16BE(0) : 0,
        hasHeader ? buf.readUInt16BE(2) : 0,
        null,
        RCODE_FORMERR,
      )
    }
    ...
    for (const rec of records) {
      ...
      const typeMatch = qtype === TYPE.ANY || wireType === typeIdToName(qtype)
      if (!typeMatch) continue

      const encoded = tryEncode(() => encodeRecord(rec, owner, apex, zone))
      if (!encoded) continue
    ...
    if (
      ...
      const soa = soaRec
        ? tryEncode(() => encodeRecord(soaRec, apex, apex, zone))
        : zone
          ? tryEncode(() =>
              buildRR(apex, TYPE.SOA, Number(zone.ttl ?? 3600), rdataSoa(zone, apex)),
            )
          : null
      if (soa) answers.push(soa)
    }
